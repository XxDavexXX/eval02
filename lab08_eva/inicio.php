            ...
            <tbody>
                <?php 
                    ...
                    while($filas=mysqli_fetch_array($res))
                    {
                        ...
                        echo "<td><button id='doc".$num."' style='margin-right:10px;' class='btn btn-info' onclick='abrirFormulario_productos_edit(".$num.")'>Editar</button><button id='doc".$num."' style='margin-right:10px;' class='btn btn-danger' onclick ='eliminar_producto(".$num.")'>Eliminar</a></button>";
                        echo "</tr>";
                        $num++;
                    }
                ?>
...
<script>
        ...
        let xhr = new XMLHttpRequest();
        xhr.open("POST", "productos/form_editar_productos.php", true);
        xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xhr.send("id=" + x);
        var ventana = window.open("productos/form_editar_productos.php?x=" + x, "_blank", "width=1000,height=700");
